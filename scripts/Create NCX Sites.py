...
CSV Format (case-insensitive). Device/group columns (priority order):
    - If present, router/device column is used: "id", "router_id", or "router id" (e.g. from NCM devices grid). One row per device; multiple rows allowed.
    ...
    Optional: site_name — if present, used as the NCX site name; otherwise the router name is used.
    ...
    for identifier, ncx_network_id, site_name_override in rows:
        ...
        for router in routers:
            ...
            if isinstance(site, str):
                ...
                # Real failure: show what the API returned (e.g. license/validation errors)
                print(
                    f'Error creating NCX site for router {router["id"]} {router["name"]}.  '
                    "Check subscriptions (router may need NCX-capable subscription / network license)."
                )
                if site is not None:
